...
class SalesRecord {
    constructor(data = {}) {
        ...
        this.id = clean(data.id) || SalesRecord.makeId(this.articleKey, this.date, this.source, this.barcode);
        ...
        this.amount = Math.max(0, number(data.amount ?? data.totalAmount));
        this.totalAmount = this.amount;
        this.importBatchId = clean(data.importBatchId) || null;
        this.createdAt = data.createdAt || new Date().toISOString();
    }

    static makeId(articleKey, date, source = 'sales', barcode = '') {
        return `${clean(source)}|${clean(articleKey)}|${clean(barcode)}|${clean(date)}`;
    }

    static createFromImport(data) {
        const record = new SalesRecord(data);
        record.id = clean(data.id) || this.makeId(record.articleKey, record.date, record.source, record.barcode);
        return record;
    }
    ...
    static aggregate(records) {
        ...
        return (records || []).reduce((acc, raw) => {
            const record = raw instanceof SalesRecord ? raw : new SalesRecord(raw);
            const id = record.id || this.makeId(record.articleKey, record.date, record.source, record.barcode);
            if (seen.has(id)) return acc;
            seen.add(id);
            acc.totalOrders += record.orders;
            acc.totalDelivered += record.delivered;
            acc.totalReturns += record.returns;
    ...
    static filterByPeriod(records, startDate, endDate) {
        ...
        if (!start || !end) return [];
        return (records || []).filter(record => { const date = dateOnly(record.date); return date >= start && date <= end; });
    }

    static filterLastDays(records, days = 30, now = new Date()) {
        const count = Math.max(1, Math.floor(number(days) || 30));
        const end = new Date(now); end.setHours(0, 0, 0, 0);
        const start = new Date(end); start.setDate(start.getDate() - (count - 1));
    ...
    static validate(record) {
        ...
        for (const field of ['orders', 'delivered', 'returns', 'amount']) if (number(record[field]) < 0) errors.push(`Отрицательное значение: ${field}`);
