...
export async function replaceMany(name,rows){const result=await replaceLocal(name,rows);if(!LOCAL_ONLY_STORES.has(name))try{await sharedRepository.mutate(name,'clear',{});for(const row of rows)await sharedRepository.mutate(name,'upsert',row)}catch{}return result}
