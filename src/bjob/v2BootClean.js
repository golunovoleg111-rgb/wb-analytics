...
const OPTIONAL_MODULES=[
  ...
  '../data/sharedDataBridge.js','./appExperience.js','./uiEnhancements.js','./dashboardExperience.js','./fbsWorkspaceUI.js','./fbsPickingUI.js','./fbsExperience.js','./fbsFixes.js','./fbsOperationsUI.js','./fbsQrExperience.js','./fbsQrFullscreenFix.js','./fbsStorageUI.js'
];
...
async function authenticate(){
  if(!session()){
    ...
    await new Promise(resolve=>root.querySelector('form').addEventListener('submit',async e=>{
      ...
      catch(err){root.querySelector('.bjob-clean-error').textContent=err?.message||'Не удалось выполнить вход.';console.error('B-JOB login failed',err);}
    }));
  }
  ...
  if(current?.mustChangePassword){
    ...
    await new Promise(resolve=>root.querySelector('form').addEventListener('submit',async e=>{
      e.preventDefault();const f=e.target;if(f.value.value!==f.repeat.value){root.querySelector('.bjob-clean-error').textContent='Пароли не совпадают.';return;}
      try{await completeFirstLogin(f.value.value);root.remove();window.dispatchEvent(new CustomEvent('bjob:auth-ready'));resolve();}
      catch(err){root.querySelector('.bjob-clean-error').textContent=err?.message||'Не удалось сохранить пароль.';}
    }));
  }
}
...
async function registerServiceWorker(){
  ...
  try{
    ...
    return registration;
  }catch(error){console.warn('B-JOB service worker unavailable; continuing online/offline-local.',error);return null;}
}

function status(){
  const old=document.querySelector('.bjob-clean-status');if(old)old.remove();if(!session())return;
