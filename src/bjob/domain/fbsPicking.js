...
export function scanShipmentItem(shipment,item,{boxId='',barcode=''}={}){
  ...
  const target=shipment.items.find(x=>x.variantId===key||Boolean(barcode)&&x.barcode===barcode);
...
export function addLabel(shipment,variantId,labelCode){if(shipment.status!=='labeling')throw new Error('Поставка не находится на этапе маркировки.');const key=clean(variantId);const item=shipment.items.find(x=>x.variantId===key);if(!item)throw new Error('Товар отсутствует в поставке.');const code=clean(labelCode);if(!code)throw new Error('Код WB-стикера не может быть пустым.');if((shipment.labels||[]).some(x=>x.labelCode===code))throw new Error('Этот WB-стикер уже зарегистрирован.');const already=(shipment.labels||[]).filter(x=>x.variantId===key).length;const required=item.quantity;if(already>=required)throw new Error('Все единицы этого товара уже промаркированы.');return {...shipment,labels:[...(shipment.labels||[]),{variantId:key,labelCode:code,printedAt:now()}],updatedAt:now()};}
