...
export function createFbsSpace({id,name,address='',rows=1,positionsPerRow=1,maxLevels=6}={}){
  ...
  const r=positive(rows,'Количество рядов');
  const p=positive(positionsPerRow,'Количество позиций');
  const l=positive(maxLevels,'Количество уровней');
...
export function validateBoxPlacement(box,space,boxes=[]){
  ...
  if(collision)throw new Error(`Место уже занято коробкой ${collision.name||collision.id}.`);
  return true;
}

export function getBlockingBoxes(box,boxes=[]){
  const {row,position,level}=box.coordinates||{};
  return boxes.filter(x=>x.id!==box.id&&x.spaceId===box.spaceId&&x.coordinates?.row===row&&x.coordinates?.position===position&&Number(x.coordinates?.level)>Number(level)&&x.status!=='disabled').sort((a,b)=>Number(a.coordinates.level)-Number(b.coordinates.level));
}
...
export function addToFbsBox(box,item,quantity=1){
  const q=positive(quantity,'Количество');
  if(!q)throw new Error('Количество должно быть больше нуля.');
  const current=box.items.reduce((sum,x)=>sum+Number(x.quantity||0),0);
  if(box.capacity&&current+q>box.capacity)throw new Error(`В коробке недостаточно места. Свободно: ${box.capacity-current}.`);
  const key=clean(item.variantId||item.id||item.barcode||item.article);
  if(!key)throw new Error('У товара нет идентификатора варианта.');
  const items=box.items.map(x=>x.variantId===key?{...x,quantity:Number(x.quantity||0)+q}:x);
  if(!items.some(x=>x.variantId===key))items.push({...item,variantId:key,quantity:q});
  return {...box,items,updatedAt:now()};
}

export function removeFromFbsBox(box,variantId,quantity=1){
  const q=positive(quantity,'Количество');
  const key=clean(variantId);
  const existing=box.items.find(x=>x.variantId===key);
  if(!existing)throw new Error('В коробке нет этого варианта товара.');
  if(Number(existing.quantity)<q)throw new Error(`В коробке осталось только ${existing.quantity} шт.`);
  return {...box,items:box.items.map(x=>x.variantId===key?{...x,quantity:Number(x.quantity)-q}:x).filter(x=>x.quantity>0),updatedAt:now()};
}
