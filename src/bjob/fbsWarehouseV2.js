...
export function createWarehouse(name='Новый склад'){return {version:VERSION,id:uid(),name,canvas:{width:1600,height:1000},walls:[],zones:[],entrances:[],features:[],boxes:[],assemblyTasks:[],updatedAt:new Date().toISOString()};}
...
export function removeBox(w,boxId){const b=box(w,boxId);if(b.locked)throw Error('Короб заблокирован');const z=w.zones.find(x=>x.id===b.zoneId);if(z)z.boxIds=z.boxIds.filter(id=>id!==boxId);w.boxes=w.boxes.filter(x=>x.id!==boxId);touch(w)}
