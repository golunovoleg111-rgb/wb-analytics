...
/**
 * Permission boundary for warehouse consumers.
 * This module is the only public authorization API for warehouse commands.
 */
...
export function canViewWarehouse(role) {
  return can(PERMISSIONS.WAREHOUSE_VIEW, role);
}

export function canEditWarehouse(role) {
  return can(PERMISSIONS.WAREHOUSE_EDIT, role);
}

export function canPublishWarehouse(role) {
  return can(PERMISSIONS.WAREHOUSE_PUBLISH, role);
}

export function canManageZones(role) {
  return can(PERMISSIONS.WAREHOUSE_MANAGE_ZONES, role);
}

export function canManageGeometry(role) {
  return can(PERMISSIONS.WAREHOUSE_MANAGE_GEOMETRY, role);
}

export function canManageBoxes(role) {
  return can(PERMISSIONS.WAREHOUSE_MANAGE_BOXES, role);
}
