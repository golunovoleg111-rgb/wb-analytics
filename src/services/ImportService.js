// ============================================================
// BELTANEE — IMPORT SERVICE v1.7.0
// Excel/CSV -> validation -> normalization -> deterministic upsert -> IndexedDB
// Шаблоны WB не изменяются. Повторный импорт не размножает записи.
// ============================================================
...
function upsert(records, map, record) {
    if (!record?.id) return;
    if (map.has(record.id)) { records[map.get(record.id)] = { ...records[map.get(record.id)], ...record }; return; }
...
function storeForType(type) {
    return ({ [ImportTypes.NOMENCLATURE]: Database.STORES.PRODUCTS, [ImportTypes.SALES]: Database.STORES.SALES, [ImportTypes.STOCK_DAILY]: Database.STORES.STOCK_HISTORY, [ImportTypes.STOCK_CURRENT]: Database.STORES.STOCK, [ImportTypes.PRICES]: Database.STORES.PRICES, [ImportTypes.MARGIN]: Database.STORES.FINANCE, [ImportTypes.ADS]: Database.STORES.ADVERTISING })[type];
}
function canonicalId(type, record) {
    if (type === ImportTypes.SALES) return `${record.articleKey || record.productId}|${record.date}`;
    if (type === ImportTypes.STOCK_DAILY || type === ImportTypes.STOCK_CURRENT) return `${record.articleKey || record.productId}|${normalizeKey(record.warehouseName)}|${record.date}`;
    if (type === ImportTypes.NOMENCLATURE) return record.articleKey;
    if (type === ImportTypes.PRICES || type === ImportTypes.MARGIN) return normalizeKey(record.article);
    if (type === ImportTypes.ADS) return record.id;
    return record.id;
}
async function compactStore(type, incoming = []) {
    const storeName = storeForType(type); if (!storeName) return;
    const all = await Database.getAll(storeName); const map = new Map();
    for (const item of all) map.set(canonicalId(type, item), { ...item, id: canonicalId(type, item) });
    for (const item of incoming) map.set(canonicalId(type, item), { ...map.get(canonicalId(type, item)), ...item, id: canonicalId(type, item) });
    const normalized = Array.from(map.values());
    if (normalized.length !== all.length || incoming.length) await Database.replaceAll(storeName, normalized);
}

class ImportService {
    static getTemplate(type) { return TEMPLATES[type] || null; }
    static getTemplates() { return { ...TEMPLATES }; }
    static isBusy() { return Boolean(activeImport); }
    ...
    static async processFile(file, type, options = {}) {
        ...
        if (activeImport) return { success: false, busy: true, error: `Уже выполняется импорт: ${activeImport}` };
        activeImport = `${type}:${Date.now()}`;
        try {
            ...
            if (previous && !options.force) return { success: false, duplicateImport: true, error: 'Этот файл с таким содержимым уже импортирован.', importBatchId: previous.importBatchId || previous.id, fileName: previous.fileName, records: [], errors: [], preview: previous.preview || null };
            ...
            const batchId = `${type}|${fileFingerprint}`;
            const records = parsed.records.map(record => ({ ...record, id: canonicalId(type, record), importBatchId: batchId }));
            ...
            await compactStore(type, records);
        ...
        } catch (error) { console.error('[ImportService]', error); return { success: false, error: error?.message || 'Ошибка импорта', records: [], errors: [] }; }
        ...
        rows.forEach((row,index) => {
            ...
            try {
                if(type===ImportTypes.NOMENCLATURE){
                    const article=clean(valueFrom(row,['Артикул продавца','Артикул','article'])), size=clean(valueFrom(row,['Размер','Размер вещи','size'])); if(!article) throw new Error('Не найден артикул');
                ...
                if(type===ImportTypes.SALES){
                    const article=clean(valueFrom(row,['Артикул продавца','Артикул','article'])),day=date(valueFrom(row,['День','Дата','Дата заказа','Date'])); if(!article||!day) throw new Error('Не найден артикул или дата');
                    const parsed=productData(article,valueFrom(row,['Размер','Размер вещи','size']),{barcode:valueFrom(row,['Баркод','Штрихкод','barcode'])}),id=`${parsed.articleKey}|${day}`;
                    const current=records[seen.get(id)]; const incoming={id,productId:parsed.articleKey,article:parsed.article,articleKey:parsed.articleKey,productGroupKey:parsed.productGroupKey,color:parsed.color,size:parsed.size,barcode:clean(parsed.barcode),date:day,orders:number(valueFrom(row,['Заказано, шт.','Заказано шт.','Заказано','Количество заказов'])),delivered:number(valueFrom(row,['Выкупили, шт.','Выкупили шт.','Выкуплено','Выкупили'])),returns:number(valueFrom(row,['Возвраты, шт.','Возвраты','Возвратов'])),amount:number(valueFrom(row,['К перечислению за товар, руб.','К перечислению за товар','Сумма выкупа'])),totalAmount:number(valueFrom(row,['Сумма заказов минус комиссия WB, руб.','Сумма заказов минус комиссия WB','Сумма заказов, руб.'])),createdAt:new Date().toISOString()};
                ...
                if(type===ImportTypes.STOCK_DAILY){
                    const article=clean(valueFrom(row,['Артикул продавца','Артикул','article'])),size=clean(valueFrom(row,['Размер','Размер вещи','size'])),warehouse=normalizeWarehouse(valueFrom(row,['Склад','warehouse'])); if(!article||!warehouse) throw new Error('Не найден артикул или склад');
                    const parsed=productData(article,size); for(const [column,raw] of Object.entries(row)){const day=date(column);if(!day||raw==='')continue;const id=`${parsed.articleKey}|${normalizeKey(warehouse)}|${day}`;upsert(records,seen,{id,productId:parsed.articleKey,articleKey:parsed.articleKey,article:parsed.article,productGroupKey:parsed.productGroupKey,warehouseName:warehouse,warehouseType:'wb',date:day,quantity:number(raw),reserved:0,source:'stock_daily',createdAt:new Date().toISOString()});} return;
                }
                if(type===ImportTypes.STOCK_CURRENT){
                    const article=clean(valueFrom(row,['Артикул продавца','Артикул','article'])),size=clean(valueFrom(row,['Размер вещи','Размер','size'])); if(!article) throw new Error('Не найден артикул');
                    const parsed=productData(article,size),inTransitTo=number(valueFrom(row,['В пути до получателей'])),inTransitFrom=number(valueFrom(row,['В пути возвраты на склад WB'])),today=new Date().toISOString().slice(0,10);
                    for(const [column,raw] of Object.entries(row)){const warehouse=normalizeWarehouse(column);if(!warehouse||AGGREGATE_STOCK_COLUMNS.has(normalizeKey(warehouse))||NON_WAREHOUSE_COLUMNS.has(normalizeKey(warehouse))||raw==='')continue;const id=`${parsed.articleKey}|${normalizeKey(warehouse)}|${today}`;upsert(records,seen,{id,productId:parsed.articleKey,articleKey:parsed.articleKey,article:parsed.article,productGroupKey:parsed.productGroupKey,warehouseName:warehouse,warehouseType:'wb',date:today,quantity:number(raw),reserved:0,inTransitTo,inTransitFrom,source:'stock_current',createdAt:new Date().toISOString()});} return;
                }
    ...
    static async _mergeById(storeName,incoming,options={}){
        const existing=await Database.getAll(storeName),map=new Map(existing.map(item=>[item.id,item]));
    ...
    static async _saveData(records,type,options={}){
        ...
        if(type===ImportTypes.SALES){await this._mergeById(Database.STORES.SALES,records,options);return;}
        if(type===ImportTypes.STOCK_CURRENT){await Database.replaceAll(Database.STORES.STOCK,records,{onProgress:progress});return;}
        if(type===ImportTypes.STOCK_DAILY){
            await this._mergeById(Database.STORES.STOCK_HISTORY,records,options);
            const current=await Database.getAll(Database.STORES.STOCK),latest=new Map(current.map(record=>[`${record.articleKey||record.productId}|${normalizeKey(record.warehouseName)}`,record]));
            for(const record of records){const key=`${record.articleKey||record.productId}|${normalizeKey(record.warehouseName)}`,previous=latest.get(key);if(!previous||record.date>=previous.date)latest.set(key,record);}
            await Database.replaceAll(Database.STORES.STOCK,Array.from(latest.values()),{onProgress:progress}); return;
        }
        if(type===ImportTypes.PRICES){
            await this._mergeById(Database.STORES.PRICES,records,options); const products=await Database.getAll(Database.STORES.PRODUCTS),byArticle=new Map(records.map(record=>[normalizeKey(record.article),record]));
            const changed=products.map(product=>{const match=byArticle.get(normalizeKey(product.article));return match?{...product,...match}:product}); await Database.replaceAll(Database.STORES.PRODUCTS,changed); return;
        }
        if(type===ImportTypes.MARGIN){
            const products=await Database.getAll(Database.STORES.PRODUCTS),byArticle=new Map(records.map(record=>[normalizeKey(record.article),record]));
            await Database.replaceAll(Database.STORES.PRODUCTS,products.map(product=>{const match=byArticle.get(normalizeKey(product.article));return match?{...product,...match}:product}));
            await this._mergeById(Database.STORES.FINANCE,records,options); return;
        }
        if(type===ImportTypes.ADS)await this._mergeById(Database.STORES.ADVERTISING,records,options);
    }
    static downloadTemplate(type){const template=TEMPLATES[type];if(!template||typeof XLSX==='undefined')return;const workbook=XLSX.utils.book_new(),sheet=XLSX.utils.aoa_to_sheet([template.columns]);XLSX.utils.book_append_sheet(workbook,sheet,'Шаблон');XLSX.writeFile(workbook,template.filename);}
}
