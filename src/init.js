...
import { getDemoExperience, getDemoOnboardingState, completeDemoOnboarding } from './demo/demoExperience.js';
...
async function collectBackup() { const backup = { version: '1.0', createdAt: new Date().toISOString(), database: Database.DB_NAME, dbVersion: Database.DB_VERSION, stores: {} }; for (const storeName of Object.values(Database.STORES)) { try { backup.stores[storeName] = await Database.getAll(storeName); } catch { backup.stores[storeName] = []; } } return backup; }
