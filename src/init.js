...
function ensureProfilePage() {
    ...
    section.innerHTML = `<div class="page-header"><h1>👤 Личный кабинет</h1><span style="font-size:12px;color:var(--text-secondary);">Локальный профиль BELTANEE v6.1</span></div>
    ...
    <div class="card"><div class="card-title">Контроль данных</div><div id="dataIntegrityInfo" style="font-size:13px;color:var(--text-secondary);">Проверяем…</div><div style="margin-top:12px;font-size:12px;color:var(--text-secondary);">База: ${escapeHtml(Database.DB_NAME)}</div></div></div>`;
    document.querySelector('main.content')?.appendChild(section);

    const profile = JSON.parse(localStorage.getItem('beltanee-profile') || '{}');
    const name = document.getElementById('profileName'); const business = document.getElementById('profileBusiness');
    ...
    document.getElementById('resetDataBtn')?.addEventListener('click', async () => {
        ...
        window.showToast?.('Данные v6.1 очищены. Можно импортировать отчёты заново.', 'success'); setTimeout(() => window.location.reload(), 600);
    });
    ...
    Promise.all([Database.count(Database.STORES.PRODUCTS), Database.count(Database.STORES.SALES), Database.count(Database.STORES.STOCK), Database.count(Database.STORES.ADVERTISING)]).then(([products, sales, stock, ads]) => {
        const el = document.getElementById('profileStorageInfo'); if (el) el.innerHTML = `Товары: <strong>${products}</strong><br>Продажи: <strong>${sales}</strong><br>Остатки: <strong>${stock}</strong><br>Реклама: <strong>${ads}</strong>`;
    }).catch(() => {});
}

function patchProfileButton() {
    const button = document.getElementById('profileBtn'); if (!button || button.dataset.beltaneePatched) return;
    const replacement = button.cloneNode(true); replacement.dataset.beltaneePatched = '1';
    replacement.addEventListener('click', () => { ensureProfilePage(); window.navigateTo?.('profile'); }); button.replaceWith(replacement);
}
...
async function checkArchitecture() {
    ...
    try {
        ...
        console.log('[BELTANEE v6.1] База готова:', { products: products.length, sales: sales.length, stockProducts: Object.keys(stock).length, integrity, database: Database.DB_NAME });
        if (!integrity.ok) console.warn('[BELTANEE v6.1] Критические проблемы данных:', integrity.warnings);
        return true;
    } catch (error) {
        console.error('[BELTANEE v6.1] Ошибка инициализации:', error); window.showToast?.(`Ошибка базы данных: ${error.message}`, 'error'); return false;
    }
}

async function init() {
    showLoading('Запускаем BELTANEE…'); document.title = 'BELTANEE v6.1 — Аналитика бизнеса на Wildberries';
    document.querySelector('.logo-text')?.replaceChildren(document.createTextNode('BELTANEE')); const badge = document.querySelector('.logo-badge'); if (badge) badge.textContent = 'v6.1';
    ensureProfilePage(); patchProfileButton(); setLoadingMessage('Загружаем разделы аналитики…'); installV61Pages();
    const ready = await checkArchitecture(); setLoadingMessage(ready ? 'Рабочее пространство готово' : 'Завершение запуска…');
    await new Promise(resolve => setTimeout(resolve, ready ? 350 : 700)); hideLoading(); console.log('✅ BELTANEE v6.1 готов');
}
