# B-JOB shared API

The server provides the persistent shared data API used by the frontend.

## Run

```bash
cd server
npm start
```

Default port: `8787`.

## API

- `GET /api/health`
- `GET /api/{store}`
- `POST /api/{store}`

Stores: products, stocks, warehouses, shipments, fbsBoxes, fbsOrders, production, users, audit.

The frontend uses same-origin `/api` by default. If the API is deployed separately, set `window.__BJOB_API_BASE__` before the application modules load, for example:

```html
<script>window.__BJOB_API_BASE__='https://api.example.com'</script>
```

Do not put credentials or database secrets into the frontend. The server-side `data/` directory must be persistent in the deployment environment.
